...
/* ── Generate By Name (streaming SSE) ── */
app.post("/generate-by-name", async (req, res) => {
  try {
    ...
    const cached = getCache(cacheKey);
    if (cached) {
      // Instant cache hit — stream it back so client code is uniform
    ...
    const filterLines = [];
    if (filters.cuisine?.length)   filterLines.push(`Cuisine: ${filters.cuisine.join(", ")}`);
    if (filters.diet?.length)      filterLines.push(`Diet: ${filters.diet.join(", ")}`);
    ...
    const prompt = `Recipe for: "${recipeName}"${filterBlock}
Steps need "time_min". RETURN VALID JSON ONLY:
{"overview":"","difficulty_label":"","servings":"","prep_time":"","cook_time":"","ingredients":{"main":[{"name":"","quantity":"","qty_number":0,"unit":""}]},"steps":[{"text":"","time_min":5}],"nutrition":{"calories":"","protein":"","carbs":"","fat":""},"filter_notes":""}`;
    ...
    const stream = await openai.chat.completions.create({
      ...
      stream: true,
      messages: [
        { role: "system", content: `You are a professional chef and nutritionist.${langLine}` },
        { role: "user", content: prompt },
      ],
    });
    ...
    // Parse and cache on completion
    try {
      const parsed = JSON.parse(accumulated.replace(/```json|```/g, "").trim());
      if (Array.isArray(parsed.steps)) {
        parsed.steps = parsed.steps.map(s => typeof s === "string" ? { text: s, time_min: null } : s);
      }
      const result = { ...parsed, _title: recipeName.trim() };
      setCache(cacheKey, result);
    ...
    console.error("generate-by-name-stream:", err);
    if (!res.headersSent) res.status(500).json({ error: "Failed to generate recipe" });
    else res.end();
  }
});
  ...
  Promise.all(
    titles.map(async (recipeName) => {
      ...
      try {
        ...
        console.error(`Prefetch failed [${recipeName}]:`, err.message);
      }
    })
  ).catch(() => {});
});
